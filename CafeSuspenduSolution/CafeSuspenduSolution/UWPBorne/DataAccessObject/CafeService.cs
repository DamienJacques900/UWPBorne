    ...
    class CafeService
    {
        public async Task<IEnumerable<CafeWithCharities>> GetCafe()
        {
            try
            {
                ...
                var cafe = rawCafes["data"][0].Children().Select(d => new CafeWithCharities()
                {
        ...
        public async Task ProceedBooking(string cafeName, int terminalId, string bookingName)
        {
            try
            {
                PushBookingInformations informations = new PushBookingInformations()
                {
                    CafeName = cafeName,
                ...
                var json = JsonConvert.SerializeObject(informations);
